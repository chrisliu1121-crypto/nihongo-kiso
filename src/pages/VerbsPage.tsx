...
const POTENTIAL_EXAMPLES: Example[] = [
  ...
  },
  {
    ...
    translation: "這裡可以拍照嗎？",
    note: "問規定或條件上能不能拍；想禮貌地徵求許可，說 撮ってもいいですか",
  },
];
...
const VOLITIONAL_EXAMPLES: Example[] = [
  ...
  },
  {
    tokens: [
      ...
      P("と", "（引用）"),
      { surface: "した", reading: "した", gloss: "（〜とする：正要）" },
    ...
    <div className="space-y-8">
      ...
      <section id="potential" className="scroll-mt-[calc(var(--nav-h)+1rem)] space-y-4">
        ...
        <ul className="list-inside list-disc space-y-1.5 text-sm text-stone-700">
          ...
          <li>
            原本用 を 的受詞，改成可能形後常換成 <strong>が</strong>：日本語を話す → 日本語<strong>が</strong>話せる（這類用 を 也聽得到，が 較常見）。
            する → できる 則只用 が：日本語<strong>が</strong>できる。に・で・へ 等其他助詞不變。
          </li>
          <li>
            口語常把一段動詞和来る的 られる 說成 れる（「ら抜き言葉」）：見れる、食べれる、来れる。聽得懂就好，考試和正式場合用 られる。
          </li>
          <li>
            一段動詞和来る的可能形，與被動形（也是尊敬形）長得一樣：食べられる 可以是「能吃」，也可以是「被吃」，要看上下文。
          </li>
          <li>
            有些五段動詞的可能形剛好和別的動詞同形：並ぶ → 並べる（能排隊）＝ 並べる（排列）；着く → 着ける（能抵達）＝ 着ける（つける，穿戴）。同樣靠上下文分辨。
          </li>
          <li>
